...
* @returns {number}
*/
export function wadd(input) {
    const ret = wasm.wadd(addHeapObject(input));
    return ret;
}
...
* @returns {number}
*/
export function wfib(n) {
    const ret = wasm.wfib(n);
    return ret >>> 0;
}
...
* @returns {number}
*/
export function fib_witer(n) {
    const ret = wasm.fib_iter(n);
    return ret >>> 0;
}
...
* @returns {string}
*/
export function w_op_string_builder(str, args) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        const ptr0 = passStringToWasm0(str, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        wasm.w_op_string_builder(retptr, ptr0, len0, addHeapObject(args));
...
const wasm_url = new URL('comp_bg.wasm', import.meta.url);
let wasmCode = '';
